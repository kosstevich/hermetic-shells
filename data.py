...
class Data:
    def __init__(self, filename:str = "data/sample-data.xlsx", sheet_name:str = "Лист1"):
        self.input_data, self.data = self._clear_data(pd.read_excel(filename, sheet_name))
        
        print(self.input_data.info())
        print(self.input_data.head(50))
        print(self.input_data.tail(50))
        print(self.data)

    def _clear_data(self, df):
        """Здесь данные очищаются под конкретный шаблон из пробных данных(КГО 5 блок 2019).
